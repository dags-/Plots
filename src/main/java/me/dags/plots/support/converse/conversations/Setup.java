package me.dags.plots.support.converse.conversations;
 ...
 */
public class Setup implements Conversation {

    private final ConversationSpec conversation = worldConversation(1, TimeUnit.MINUTES);

    @Override
    public void startConversation(CommandSource source) {
        try {
    ...
    private static ConversationSpec worldConversation(int time, TimeUnit unit) {
        ...
        return ConversationSpec.builder()
                .onComplete(conversation -> conversation.getSource().ifPresent(src -> {
                    String world = conversation.getContext().<String>getLast("world_name", "name").orElse("world-" + System.currentTimeMillis());
